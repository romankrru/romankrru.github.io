...
Агент показывает мне предлагаемое разбиение, получает фидбек, итерирует и потом публикует дочерние issue на Github с лейблом `ready-for-agent` — это сигнал для ralph, что задачу можно брать в работу.

На все issue, относящиеся к одному PRD, вешается схема из двух лейблов:
