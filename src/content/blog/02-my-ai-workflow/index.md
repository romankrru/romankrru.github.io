...
Удобный, но хрупкий режим — реализовать фичу в одной чат-сессии: сначала обсудить, потом написать код, потом чинить CI в том же окне. Контекст быстро раздувается, а на очень длинном окне (100k токенов и выше) модель хуже помнит ранние договорённости и чаще ошибается в мелочах. Отсюда знакомые симптомы: термины плывут, scope расползается, откатить решение сложно — всё перемешано в одной ленте.

Главный тезис этой статьи простой: агент справляется там, где решения уже приняты и записаны, контекст каждой итерации мал, а состояние работы живёт вне агента — в issue-трекере или файлах, а не в его памяти. Ниже — схема, как я провожу фичу от идеи до PR с этими ограничениями.
...
Кроме `CONTEXT.md`, `/grill-with-docs` иногда предлагает завести ADR — отдельный markdown-файл в `docs/adr/`. ADR заводится, если выполняются три условия: решение тяжело откатить, оно выглядит странно без контекста, имеет какой-то компромисс. Таких документов сознательно мало, реализация здесь не описывается.
...
Агент показывает предлагаемое разбиение, получает фидбек, итерирует и потом публикует дочерние issue на GitHub с лейблом `ready-for-agent` — по этому лейблу ralph отбирает задачи, которые можно брать в работу.
