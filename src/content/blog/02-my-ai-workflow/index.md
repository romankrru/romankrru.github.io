...
У меня есть скрипт .agents/ralph/loop.sh (TODO: link), передает ему один и тот же PROMPT.md (todo:link) и смотрит на последнюю строку вывода: STATUS=done | STATUS=progress | STATUS=blocked. По статусу решает: выходим или зовём агента снова.

(TODO: кратко описать из чего состоит скрипт, зачем флаг -p, как запускать через opencode run)
...
- Реализует ишью через TDD — это отдельный скилл tdd (TODO: link). Используются vertical slice: один тест → минимальная имплементация → следующий тест. Acceptance criteria из тела ишью становятся списком тестов.
...
огда ralph закрывает последнего ребёнка PRD, у меня в GitHub лежит один большой epic-PR prd/<N>-<slug> → main, состоящий из аккуратной серии сквош-коммитов «один коммит на ишью».

Пример итогового child-PR в epic: PR #18 — Configured round size: Settings page and home navigation. (TODO: link)
