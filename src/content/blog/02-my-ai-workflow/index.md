...
В небольшой [игре-квизе по флагам](https://geo-quiz-phi.vercel.app/) нужно добавить экран настроек (`/settings`): игрок задаёт количество вопросов в раунде — пресеты (10, 25, все страны каталога) или своё число; выбор сохраняется в `localStorage` и применяется при старте следующего раунда.
...
По моему опыту, AI может нагенерировать в `CONTEXT.md` слоп, который выглядит правдоподобно. Поэтому этот этап требует большой ответственности от разработчика. А на code review такую проблему тяжело выявить: ревьюеру нужно знание и погружение в доменную область.
