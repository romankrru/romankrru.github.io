...
У меня есть скрипт `.agents/ralph/loop.sh` (_TODO: link_), передает ему один и тот же `PROMPT.md` (_TODO: link_) и смотрит на последнюю строку вывода: `STATUS=done` | `STATUS=progress` | `STATUS=blocked`. По статусу решает: выходим или зовём агента снова.

Рядом с ним лежит `loop-once.sh` (_TODO: link_) — та же логика, но ровно одна итерация без цикла: запустил, посмотрел diff и логи, решил, что делать дальше. На практике им я пользуюсь чаще, чем полным `loop.sh` — это и есть «части вместо целого» из оговорки в начале.
